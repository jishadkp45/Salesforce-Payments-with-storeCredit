    ...
    try {
        ...
        Transaction.wrap(function () {
            ...
            if (customer.profile !== null) {
                ...
                var totalPrice = currentBasket.totalGrossPrice.value;
                if (paymentInstruments.length > 1) {
                    var paymentInstrumentStoreCredit = paymentInstruments[0];
                    var paymentInstrumentCreditCard = paymentInstruments[1];
                    var storeCreditAmount = new Money(storeCreditBalance, currentBasket.currencyCode);
                    var creditCardAmount = new Money(totalPrice - storeCreditBalance, currentBasket.currencyCode);
                    paymentInstrumentStoreCredit.paymentTransaction.setAmount(storeCreditAmount);
                    paymentInstrumentCreditCard.paymentTransaction.setAmount(creditCardAmount);
                } else {
